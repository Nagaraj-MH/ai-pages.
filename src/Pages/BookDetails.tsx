...
const BookDetails = () => {
  ...
  const handleAddComment = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:8080/api/v1/books/${bookId}/comment`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ book_id:bookId,content: newComment, user_id: 1 }),
      });
      if (!response.ok) {
    ...
    <div className={`min-h-screen flex flex-col ${darkMode ? "dark bg-gray-900 text-white" : "bg-white text-black"}`}>
      <div className="container mx-auto px-6 py-12 flex flex-col md:flex-row gap-8">
        <aside className="w-full md:w-1/4 flex flex-col items-center">
          <img src={`http://localhost:8080/api/v1/books/${bookId}/cover`} alt={book.title} className="w-48 h-64 rounded shadow-md mb-4" />
